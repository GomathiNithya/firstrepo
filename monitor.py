...
if args.options == "elastic-ip":#associated and unassociated Volumes
    ...
    if args.region:
        ...
        if len(ass_elastic_Ip) == 0:
            ...
            for volume in range(len(ass_elastic_Ip)):
                vol=connect.describe_addresses(Filters=[{'Name':'public-ip','Values':[ass_elastic_Ip[volume]]}])
                for volu in vol['Addresses']:
                    print("\n\n",regionsName[i],"\t",zonetag[volume],"\t\t",volu['PublicIp'],"\t",volu['AllocationId'],"\t",volu['InstanceId'],"\t\t",volu['NetworkInterfaceId'],"\t\t",volu['Domain'])
        ...
        if len(unass_elastic_Ip) == 0:
            ...
            for volume in range(len(unass_elastic_Ip)):
                vol=connect.describe_addresses(Filters=[{'Name':'public-ip','Values':[unass_elastic_Ip[volume]]}])
                for volu in vol['Addresses']:
                    print("\n\n",regionsName[i],"\t",zoneuntag[volume],"\t\t\t",volu['PublicIp'],"\t\t",volu['AllocationId'],"\t\t",volu['Domain'])
    else:
        for i in range(0,10):
            connects=boto3.resource('ec2',regions[i]);
            connect=boto3.client('ec2',regions[i]);
